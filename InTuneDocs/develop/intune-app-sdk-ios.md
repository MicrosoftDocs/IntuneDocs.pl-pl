...
Zestaw SDK aplikacji usługi Microsoft Intune dla systemu iOS umożliwia wdrożenie zasad ochrony aplikacji usługi Intune w aplikacji dla systemu iOS. Aplikacja z obsługą funkcji zarządzania aplikacjami mobilnymi (MAM, Mobile Application Management) jest zintegrowana z zestawem SDK aplikacji usługi Intune, co pozwala administratorom IT wdrażać zasady w aplikacji mobilnej, gdy jest ona aktywnie zarządzana przez funkcję MAM usługi Intune.
...
## <a name="building-the-sdk-into-your-mobile-app"></a>Kompilowanie aplikacji mobilnej przy użyciu zestawu SDK
...
13. Użyj opcji konsolidatora `-force_load PATH_TO_ADAL_LIBRARY` podczas ustanawiania połączenia z biblioteką.
    ...
    Dodaj składnik `-force_load {PATH_TO_LIB}/libADALiOS.a` do ustawienia konfiguracji kompilacji `OTHER_LDFLAGS` lub pozycji „Other Linker Flags” (Inne flagi konsolidatora) w interfejsie użytkownika. Element `PATH_TO_LIB` należy zastąpić lokalizacją plików binarnych biblioteki ADAL.

## <a name="configure-azure-directory-authentication-library-adal"></a>Konfigurowanie biblioteki uwierzytelniania usługi Azure Directory (ADAL)
...
Zazwyczaj biblioteka ADAL wymaga od aplikacji zarejestrowania się w usłudze [Azure Active Directory] i uzyskania unikatowego identyfikatora (ClientID) oraz innych identyfikatorów w celu zagwarantowania bezpieczeństwa tokenów przyznanych aplikacji. Zestaw SDK aplikacji usługi Intune używa domyślnych wartości rejestracji podczas kontaktowania się z usługą Azure Active Directory.  
...
## <a name="register-your-app-with-intune-mam-service"></a>Rejestrowanie aplikacji w usłudze Intune MAM
...
### <a name="de-register-accounts"></a>Anulowanie rejestracji kont
...
## <a name="enrolling-without-prior-sign-in"></a>Rejestrowanie bez wcześniejszego zalogowania

Aplikacja, która nie loguje użytkownika za pomocą usługi Azure Active Directory, może nadal otrzymywać zasady zarządzania aplikacjami mobilnymi z usługi Intune, wywołując poniższy interfejs API, aby zestaw SDK obsłużył to uwierzytelnianie. Aplikacja powinna używać tego interfejsu, jeśli nie uwierzytelniła użytkownika za pomocą usługi AAD, ale nadal musi on pobrać zasady zarządzania aplikacjami mobilnymi, aby chronić swoje dane w aplikacji — na przykład jeśli do logowania w aplikacji jest używana inna usługa uwierzytelniania albo aplikacja w ogóle nie obsługuje logowania. W tym celu aplikacja powinna wywołać metodę `loginAndEnrollAccount` znajdującą się w elemencie **Headers/IntuneMAMEnrollment.h**:
...
## <a name="implementing-save-as-controls"></a>Implementowanie kontrolek Zapisz jako
...
## <a name="configure-intune-app-sdk-settings"></a>Konfigurowanie ustawień zestawu SDK aplikacji usługi Intune
...
Tożsamość jest po prostu nazwą użytkownika konta (np. user@contoso.com).). Deweloperzy mogą ustawiać tożsamość aplikacji na następujących różnych poziomach:
...
## <a name="recommended-ios-best-practices"></a>Zalecane najlepsze rozwiązania dla systemu iOS
...
Tak, administrator IT może wysłać do aplikacji polecenie selektywnego czyszczenia danych, co spowoduje wyrejestrowanie użytkownika i wyczyszczenie danych użytkownika.  Zestaw SDK automatycznie obsługuje ten scenariusz i będzie wysyłać powiadomienia za pośrednictwem metody delegata un-enroll.

## <a name="submitting-your-app-to-the-app-store"></a>Przesyłanie aplikacji do sklepu z aplikacjami
