...
Te ustawienia dotyczą typów rejestracji systemu Android Enterprise, w przypadku których usługa Intune steruje całym urządzeniem (np. w pełni zarządzane lub dedykowane urządzenia z systemem Android Enterprise).
...
- **Zmiany głośności**: pozycja **Blokuj** uniemożliwia użytkownikom zmienianie głośności urządzenia oraz powoduje wyciszenie głównego poziomu głośności. Pozycja **Nieskonfigurowane** umożliwia korzystanie z ustawień głośności na urządzeniu.
...
- **Pomiń wskazówki podczas pierwszego użycia**: opcja **Włącz** powoduje ukrycie lub pominięcie sugestii z aplikacji zawierających samouczki krok po kroku, a także wskazówek podczas uruchamiania aplikacji. Po ustawieniu opcji **Nieskonfigurowane** zostanie użyte domyślne ustawienie systemu operacyjnego, co może prowadzić do wyświetlania tych sugestii podczas uruchamiania aplikacji.
...
- **Pojedyncza aplikacja**: użytkownicy mogą uzyskiwać dostęp tylko do jednej aplikacji na urządzeniu. Po uruchomieniu urządzenia zostanie uruchomiona tylko określona aplikacja. Użytkownicy nie mogą otwierać nowych aplikacji ani zmieniać uruchomionej aplikacji.
  ...
  - **Wybierz zarządzaną aplikację**: Wybierz z listy zarządzaną aplikację ze sklepu Google Play.
  ...
  > [!IMPORTANT]
  > Aplikacje telefoniczne mogą nie działać prawidłowo w przypadku używania trybu kiosku z jedną aplikacją. 
...
- **Wiele aplikacji**: użytkownicy mogą uzyskiwać dostęp do ograniczonego zestawu aplikacji na urządzeniu. Po uruchomieniu urządzenia zostaną uruchomione tylko dodane aplikacje. Można również dodać linki internetowe, które użytkownicy będą mogli otwierać. Po zastosowaniu zasad użytkownicy widzą ikony dozwolonych aplikacji na ekranie głównym.
  ...
  >
  > Aplikacje telefoniczne mogą nie działać prawidłowo w przypadku używania trybu kiosku z wieloma aplikacjami. 
  ...
  - **Wirtualny przycisk ekranu głównego**: przycisk programowy kierujący użytkowników z powrotem do aplikacji Zarządzany ekran główny, aby umożliwić przełączanie się między aplikacjami. Dostępne opcje:

    - **Nieskonfigurowane** (wartość domyślna): przycisk ekranu głównego nie jest wyświetlany. Aby przełączać się między aplikacjami, użytkownicy muszą używać przycisku Wstecz.
    - **Szybkie przesunięcie w górę**: przycisk ekranu głównego jest wyświetlany, gdy użytkownik szybko przesuwa palcem w górę na urządzeniu.
    - **Przestawne**: powoduje wyświetlanie trwałego, przestawnego przycisku ekranu głównego na urządzeniu.
  ...
  - **Wyjdź z trybu kiosku**: wybierz pozycję **Włącz**, aby umożliwić administratorom tymczasowe wstrzymywanie trybu kiosku w celu zaktualizowania urządzenia. Aby skorzystać z tej funkcji, administrator wykonuje następujące czynności:
    ...
    3. Po zakończeniu wybiera aplikację **Zarządzany ekran główny**. Ten krok powoduje ponowne zablokowanie urządzenia w trybie kiosku z wieloma aplikacjami.

      Po ustawieniu opcji **Nieskonfigurowane** administratorzy nie mogą wstrzymywać trybu kiosku. Jeśli administrator wybiera przycisk Wstecz, a następnie przycisk **Wyjdź z kiosku**, pojawia się komunikat z informacją o tym, że kod dostępu jest wymagany.
  ...
  - **Konfiguracja sieci Wi-Fi**: opcja **Włącz** powoduje wyświetlanie kontrolki sieci Wi-Fi w aplikacji Zarządzany ekran główny oraz umożliwia użytkownikom końcowym łączenie urządzenia z różnymi sieciami Wi-Fi. Włączenie tej funkcji powoduje włączenie lokalizacji urządzenia. Opcja **Nieskonfigurowane** (domyślne) powoduje, że kontrolka Wi-Fi nie jest wyświetlana w aplikacji Zarządzany ekran główny. Uniemożliwia to użytkownikom łączenie się z sieciami Wi-Fi przy użyciu aplikacji Zarządzany ekran główny.

  - **Konfiguracja Bluetooth**: opcja **Włącz** powoduje wyświetlanie kontrolki Bluetooth w aplikacji Zarządzany ekran główny i umożliwia użytkownikom końcowym parowanie urządzeń przez połączenie Bluetooth. Włączenie tej funkcji powoduje włączenie lokalizacji urządzenia. Opcja **Nieskonfigurowane** (domyślne) powoduje, że kontrolka Bluetooth nie jest wyświetlana w aplikacji Zarządzany ekran główny. Uniemożliwia to użytkownikom konfigurowanie urządzeń Bluetooth i parowanie urządzeń podczas korzystania z aplikacji Zarządzany ekran główny.

  - **Dostęp do latarki**: opcja **Włącz** powoduje wyświetlanie kontrolki latarki w aplikacji Zarządzany ekran główny oraz umożliwia użytkownikom końcowym włączanie i wyłączanie latarki. Opcja **Nieskonfigurowane** (domyślne) powoduje, że kontrolka latarki nie jest wyświetlana w aplikacji Zarządzany ekran główny. Uniemożliwia to użytkownikom używanie latarki podczas korzystania z aplikacji Zarządzany ekran główny.

  - **Regulacja głośności multimediów**: opcja **Włącz** powoduje wyświetlanie kontrolki głośności multimediów w aplikacji Zarządzany ekran główny oraz umożliwia użytkownikom końcowym dostosowywanie głośności multimediów urządzenia przy użyciu suwaka. Opcja **Nieskonfigurowane** (domyślne) powoduje, że kontrolka głośności multimediów nie jest wyświetlana w aplikacji Zarządzany ekran główny. Uniemożliwia to użytkownikom dostosowywanie głośności multimediów urządzenia podczas korzystania z aplikacji Zarządzany ekran główny, chyba że obsługują to przyciski ich sprzętu. 

  - **Tryb wygaszacza ekranu**: Opcja **Włącz** powoduje wyświetlanie wygaszacza ekranu w aplikacji Zarządzany ekran główny, gdy urządzenie zostanie zablokowane lub upłynie limit czasu. Opcja **Nieskonfigurowane** (domyślne) powoduje, że wygaszacz ekranu nie jest wyświetlany w aplikacji Zarządzany ekran główny.

    Gdy ta opcja jest włączona, należy też skonfigurować następujące ustawienia:

    - **Ustaw niestandardowy obraz wygaszacza ekranu**: wprowadź adres URL niestandardowego pliku PNG, JPG, JPEG, GIF, BMP, WebP lub ICOimage. Wprowadź na przykład:

      - `http://www.contoso.com/image.jpg`
      - `www.contoso.com/image.bmp`
      - `https://www.contoso.com/image.webp`

      Jeśli nie wprowadzisz adresu URL, zostanie użyty domyślny obraz urządzenia, o ile taki istnieje.
      
      > [!TIP]
      > Obsługiwane są wszelkie adresy URL zasobów plików, które można przekształcić w mapę bitową.

    - **Liczba sekund wyświetlania przez urządzenie wygaszacza ekranu przed wyłączeniem ekranu**: wybierz, jak długo urządzenie ma wyświetlać wygaszacz ekranu. Wprowadź wartość z zakresu 0–9999999 sekund. Wartość domyślna to `0` sekund. Jeśli pole pozostanie puste lub zostanie ustawione na zero (`0`), wygaszacz ekranu będzie aktywny do czasu interakcji użytkownika z urządzeniem.
    - **Liczba sekund braku aktywności urządzenia przed wyświetleniem wygaszacza ekranu**: określ czas bezczynności urządzenia przed wyświetleniem wygaszacza ekranu. Wprowadź wartość z zakresu 1–9999999 sekund. Wartość domyślna to `30` sekund. Należy wprowadzić liczbę większą od zera (`0`).
    - **Wykrywaj multimedia przed uruchomieniem wygaszacza ekranu**: opcja **Włącz** (domyślne) powoduje, że wygaszacz ekranu nie jest wyświetlany podczas odtwarzania dźwięku lub wideo na urządzeniu. Opcja **Nieskonfigurowane** powoduje wyświetlanie wygaszacza ekranu nawet w przypadku odtwarzania dźwięku lub wideo.

### <a name="device-password-settings"></a>Ustawienia haseł urządzeń

- **Wyłącz ekran blokady**: Wybierz pozycję **Wyłącz**, aby uniemożliwić użytkownikom używanie funkcji blokady ekranu Blokada klawiatury na urządzeniu. Pozycja **Nieskonfigurowane** zezwala użytkownikowi na korzystanie z funkcji blokady klawiatury.
- **Wyłączone funkcje ekranu blokady**: jeśli blokada klawiatury jest włączona na urządzeniu, wybierz, które funkcje mają zostać wyłączone. Na przykład po zaznaczeniu opcji **Zabezpiecz aparat** funkcja aparatu zostanie wyłączona na urządzeniu. Wszystkie funkcje, które nie są zaznaczone, są włączone na urządzeniu.
...
- **Wymagany typ hasła**: określ typ hasła wymagany dla urządzenia. Dostępne opcje:
  ...
  - **Słabe zabezpieczenia biometryczne**: [Silne i słabe elementy biometryczne](https://android-developers.googleblog.com/2018/06/better-biometrics-in-android-p.html) (zostanie otwarta witryna internetowa systemu Android).
  - **Numeryczne**: Hasło może się składać tylko z cyfr, na przykład `123456789`. Podaj **minimalną długość hasła**, które musi wprowadzić użytkownik (od 4 do 16 znaków).
...
- **Czas do zablokowania ekranu**: wprowadź maksymalny czas do momentu zablokowania urządzenia, jaki może ustawić użytkownik. Jeśli na przykład określisz ustawienie **10 minut**, użytkownicy będą mogli ustawić czas od 15 sekund do 10 minut. W przypadku ustawienia **Nieskonfigurowane** (domyślne) usługa Intune nie zmienia tego ustawienia ani nie steruje nim.
...
- **Usuwanie użytkownika**: wybierz pozycję **Blokuj**, aby uniemożliwić użytkownikom usuwanie użytkowników. Pozycja **Nieskonfigurowane** (ustawienie domyślne) umożliwia użytkownikom usuwanie innych użytkowników z urządzenia.
- **Zmiany dotyczące konta** (tylko dedykowane urządzenia): wybierz pozycję **Blokuj**, aby uniemożliwić użytkownikom modyfikowanie kont. Pozycja **Nieskonfigurowane** (domyślna) umożliwia użytkownikom aktualizowanie kont użytkowników na urządzeniu.

  > [!NOTE]
  > To ustawienie nie jest uznawane na urządzeniach właściciela (w pełni zarządzanych). Jeśli to ustawienie zostanie skonfigurowane, będzie ignorowane i nie będzie miało żadnego wpływu.

- **Użytkownik może konfigurować poświadczenia**: opcja **Blokuj** uniemożliwia użytkownikom konfigurowanie certyfikatów przypisanych do urządzeń, nawet w przypadku urządzeń, które nie są skojarzone z kontem użytkownika. Opcja **Nieskonfigurowane** może umożliwić użytkownikom konfigurowanie lub zmienianie poświadczeń podczas uzyskiwania dostępu do nich w magazynie kluczy. 
- **Osobiste konta Google**: opcja **Blokuj** uniemożliwia użytkownikom dodawanie osobistego konta Google do urządzenia. Opcja **Nieskonfigurowane** (domyślne) umożliwia użytkownikom dodawanie osobistego konta Google.

### <a name="applications"></a>Aplikacje

- **Zezwalaj na instalację z nieznanych źródeł**: Wybierz pozycję **Zezwalaj**, aby użytkownicy mogli włączać opcję **Nieznane źródła**. To ustawienie umożliwia instalację aplikacji z nieznanych źródeł, innych niż Sklep Google Play. Pozycja **Nieskonfigurowane** uniemożliwia użytkownikom włączanie opcji **Nieznane źródła**.
...
- **Klient sieci VPN**: wybierz klienta sieci VPN, który obsługuje opcję Zawsze włączone. Dostępne opcje:
  ...
  > - Należy skonfigurować klienta sieci VPN przy użyciu [profilu sieci VPN](vpn-settings-android-enterprise.md) lub za pośrednictwem [profilu konfiguracji aplikacji](../apps/app-configuration-policies-use-android.md).
...
- **Zalecany globalny serwer proxy**: wybierz opcję **Włącz**, aby dodać globalny serwer proxy do urządzeń. Gdy ta opcja jest włączona, ruch HTTP i HTTPS (w tym niektóre aplikacje na urządzeniu) używają wprowadzonego serwera proxy. Ten serwer proxy jest tylko zaleceniem. Niektóre aplikacje mogą nie używać tego serwera proxy. Opcja **Nieskonfigurowane** (domyślne) powoduje, że zalecany globalny serwer proxy nie jest dodawany.

  Aby uzyskać więcej informacji o tej funkcji, zobacz [setRecommendedGlobalProxy](https://developer.android.com/reference/android/app/admin/DevicePolicyManager.html#setRecommendedGlobalProxy(android.content.ComponentName,%20android.net.ProxyInfo)) (otwiera witrynę systemu Android).

  Gdy ta opcja jest włączona, wprowadź też ustawienie **Typ** dla serwera proxy. Dostępne opcje:
  ...
  - **Bezpośredni**: wybierz tę opcję, aby ręcznie wprowadzić szczegóły serwera proxy, w tym:
    ...
    - **Wykluczone hosty**: wprowadź listę nazw hostów lub adresów IP, które nie będą korzystać z serwera proxy. Ta lista może zawierać symbol wieloznaczny gwiazdki (`*`) oraz wiele hostów rozdzielonych średnikami (`;`) bez spacji. Na przykład wprowadź `127.0.0.1;web.contoso.com;*.microsoft.com`.

  - **Automatyczna konfiguracja serwera proxy**: wprowadź ustawienie **Adres URL pliku PAC** wskazujące skrypt automatycznej konfiguracji serwera proxy. Na przykład wprowadź `https://proxy.contoso.com/proxy.pac`.
...
Te ustawienia dotyczą typów rejestracji systemu Android Enterprise, w przypadku których usługa Intune steruje tylko profilem służbowym, takich jak rejestracja profilu służbowego systemu Android Enterprise na urządzeniu osobistym lub urządzeniu typu „Przynieś własne urządzenie” (BYOD, Bring Your Own Device).
...
- **Dodawanie i usuwanie kont**: wybierz pozycję **Blokuj**, aby uniemożliwić użytkownikom końcowym ręczne dodawanie lub usuwanie kont w profilu służbowym. Na przykład w przypadku wdrożenia aplikacji Gmail w profilu służbowym systemu Android można uniemożliwić użytkownikom końcowym dodawanie i usuwanie kont w tym profilu służbowym. Pozycja **Nieskonfigurowane** zezwala na dodawanie kont w profilu służbowym.  
  ...
  > [!NOTE]
  > Nie można dodawać kont Google do profilu służbowego.
...
- **Zezwalaj na widżety z aplikacji profilu służbowego**: opcja **Włącz** umożliwia użytkownikom końcowym umieszczanie widżetów udostępnianych przez aplikacje na ekranie głównym. **Nie skonfigurowano** (ustawienie domyślne) — wyłącza tę funkcję.

  Na przykład program Outlook może być zainstalowany w profilach służbowych użytkowników. W przypadku ustawienia **Włącz** użytkownicy mogą umieszczać widżet harmonogramu na ekranie głównym urządzenia.
...
- **Zapobiegaj instalowaniu aplikacji z nieznanych źródeł w profilu osobistym**: zgodnie z projektem nie można instalować aplikacji ze źródeł innych niż sklep Play na urządzeniach z profilem służbowym systemu Android Enterprise. Urządzenia z profilem służbowym są przeznaczone do obsługi dwóch profilów:

  - Profil służbowy zarządzany przy użyciu funkcji MDM.
  - Profil osobisty odizolowany od zarządzania przy użyciu funkcji MDM.

  To ustawienie umożliwia administratorom większą kontrolę nad instalowaniem aplikacji z nieznanych źródeł. Opcja **Nieskonfigurowane** (domyślne) umożliwia instalowanie aplikacji z nieznanych źródeł w profilu osobistym. Opcja **Blokuj** uniemożliwia instalowanie aplikacji ze źródeł innych niż sklep Play w profilu osobistym.
