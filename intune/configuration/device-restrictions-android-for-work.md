...
- **Zmiany woluminów**: **bloku** uniemożliwiają użytkownikom zmianę woluminu urządzenia, a także wyciszenie woluminu głównego. Pozycja **Nieskonfigurowane** umożliwia korzystanie z ustawień głośności na urządzeniu.
...
- **Wiele aplikacji**: użytkownicy mogą uzyskiwać dostęp do ograniczonego zestawu aplikacji na urządzeniu. Po uruchomieniu urządzenia zostaną uruchomione tylko dodane aplikacje. Można również dodać linki internetowe, które użytkownicy będą mogli otwierać. Po zastosowaniu zasad użytkownicy widzą ikony dozwolonych aplikacji na ekranie głównym.
  ...
  - **wirtualnego przycisku strony głównej**: przycisk miękkiego klawisza, który zwraca użytkowników do zarządzanego ekranu głównego, aby użytkownicy mogli przełączać się między aplikacjami. Dostępne opcje:

    - **Nie skonfigurowano** (wartość domyślna): przycisk główny nie jest wyświetlany. Aby przełączać się między aplikacjami, użytkownicy muszą używać przycisku Wstecz.
    - **Przesuń w górę**: przycisk Strona główna pokazuje, kiedy użytkownik szybko przesuwa się w urządzeniu.
    - **zmiennoprzecinkowe**: pokazuje trwały, swobodny przycisk na urządzeniu.
  ...
  - **Wyjdź z trybu kiosku**: wybierz pozycję **Włącz**, aby umożliwić administratorom tymczasowe wstrzymywanie trybu kiosku w celu zaktualizowania urządzenia. Aby skorzystać z tej funkcji, administrator wykonuje następujące czynności:
    ...
    3. Po zakończeniu wybierz **zarządzanym ekranem głównym** aplikacji. Ten krok powoduje ponowne zablokowanie urządzenia w trybie kiosku z wieloma aplikacjami.

      Jeśli ustawiono **nie skonfigurowane**, Administratorzy nie mogą wstrzymać trybu kiosku. Jeśli administrator wybiera przycisk Wstecz, a następnie przycisk **Wyjdź z kiosku**, pojawia się komunikat z informacją o tym, że kod dostępu jest wymagany.
  ...
  - **Konfiguracja sieci Wi-Fi**: **Enable** pokazuje formant sieci Wi-Fi na zarządzanym ekranie głównym oraz umożliwia użytkownikom końcowym łączenie urządzenia z różnymi sieciami Wi-WiFi. Włączenie tej funkcji powoduje włączenie lokalizacji urządzenia. **Nie skonfigurowano** (domyślnie) nie pokazuje kontrolki Wi-Fi na zarządzanym ekranie głównym. Uniemożliwia to użytkownikom łączenie się z sieciami Wi-Fi przy użyciu zarządzanego ekranu głównego.

  - **Konfiguracja Bluetooth**: **Enable** pokazuje kontrolkę Bluetooth na zarządzanym ekranie głównym i umożliwia użytkownikom końcowym parowanie urządzeń za pośrednictwem połączenia Bluetooth. Włączenie tej funkcji powoduje włączenie lokalizacji urządzenia. **Nie skonfigurowano** (domyślnie) nie pokazuje kontrolki Bluetooth na zarządzanym ekranie głównym. Uniemożliwia to użytkownikom konfigurowanie urządzeń Bluetooth i parowania przy użyciu zarządzanego ekranu głównego.

  - **Flashlight Access**: **Enable** pokazuje kontrolkę Flashlight na zarządzanym ekranie głównym i umożliwia użytkownikom końcowym włączenie lub wyłączenie Flashlight. **Nie skonfigurowano** (domyślnie) nie pokazuje formantu Flashlight na zarządzanym ekranie głównym. Uniemożliwia to użytkownikom korzystanie z Flashlight przy użyciu zarządzanego ekranu głównego.
  ...
  - **Tryb wygaszacza ekranu**: **Włącz** pokazuje wygaszacz ekranu na zarządzanym ekranie głównym, gdy urządzenie jest zablokowane lub przetrwają limit czasu. **Nie skonfigurowano** (domyślnie) nie jest wyświetlany wygaszacz ekranu na zarządzanym ekranie głównym.
    ...
    - **Liczba sekund, przez które urządzenie wyświetla wygaszacz ekranu przed wyłączeniem**ekranu: Wybierz, jak długo urządzenie ma wyświetlać wygaszacz ekranu. Wprowadź wartość z zakresu od 0-9999999 sekund. Wartość domyślna to `0` sekund. Jeśli pole pozostanie puste lub zostanie ustawione na `0` sekund, wygaszacz ekranu będzie aktywny do czasu interakcji użytkownika z urządzeniem.
    - **Liczba sekund, przez które urządzenie jest nieaktywne przed wyświetleniem wygaszacza ekranu**: Określ, jak długo urządzenie jest bezczynne, zanim zostanie wyświetlone wygaszacz ekranu. Wprowadź wartość z zakresu od 1-9999999 sekund. Wartość domyślna to `30` sekund. Należy wprowadzić liczbę większą od zera (`0`).
    - **Wykryj nośnik przed uruchomieniem**wygaszacz ekranu: **Włącz** (domyślnie) nie pokazuje wygaszacza ekranu, jeśli dźwięk lub wideo są odtwarzane na urządzeniu. **Nie skonfigurowano** wyświetla wygaszacz ekranu, nawet w przypadku odtwarzania dźwięku lub wideo.
...
- **Czas do zablokowania ekranu**: Wprowadź maksymalny czas, jaki użytkownik może ustawić do momentu zablokowania urządzenia. Na przykład w przypadku ustawienia tego ustawienia na **10 minut**, użytkownicy mogą ustawić czas od 15 sekund do 10 minut. Po ustawieniu na **nie skonfigurowane** (domyślnie) usługa Intune nie zmienia ani nie kontroluje tego ustawienia.
...
- **Zmiany dotyczące konta**: wybierz pozycję **Blokuj**, aby uniemożliwić użytkownikom modyfikowanie kont. Pozycja **Nieskonfigurowane** (domyślna) umożliwia użytkownikom aktualizowanie kont użytkowników na urządzeniu.
...
- **Użytkownik może skonfigurować poświadczenia**: **bloku** uniemożliwia użytkownikom konfigurowanie certyfikatów przypisanych do urządzeń, nawet urządzeń, które nie są skojarzone z kontem użytkownika. **Nie skonfigurowano** mogą umożliwić użytkownikom konfigurowanie lub zmienianie poświadczeń podczas uzyskiwania dostępu do nich w magazynie kluczy. 
- **Osobiste konta Google**: **bloku** uniemożliwia użytkownikom dodawanie do urządzenia osobistego konta Google. **Nie skonfigurowano** (domyślnie) umożliwia użytkownikom dodawanie osobistych kont Google.
...
- **Klient sieci VPN**: wybierz klienta sieci VPN, który obsługuje opcję Zawsze włączone. Dostępne opcje:
  ...
  > - Należy nadal skonfigurować klienta sieci [VPN przy użyciu](vpn-settings-android-enterprise.md)profilu sieci VPN [lub za pośrednictwem](../apps/app-configuration-policies-use-android.md)profilu konfiguracji aplikacji.
...
- **Zalecana globalna**proxy: wybierz pozycję **Włącz**, aby dodać globalny serwer proxy do urządzeń. Po włączeniu ruchu HTTP i HTTPS, w tym niektórych aplikacji na urządzeniu, użyj wprowadzonego serwera proxy. Ten serwer proxy jest tylko rekomendacją. Niektóre aplikacje nie będą korzystać z serwera proxy. **Nie skonfigurowano** (domyślnie) nie dodaje zalecanego globalnego serwera proxy.
  ...
  - **bezpośrednia**: Wybierz tę opcję, aby ręcznie wprowadzić szczegóły serwera proxy, w tym:
    - **hosta**: Wprowadź nazwę hosta lub adres IP serwera proxy. Na przykład wprowadź adres `proxy.contoso.com` lub `127.0.0.1`.
  ...
  - **automatycznej konfiguracji serwera proxy**: wprowadź **adres URL certyfikatu PAC** do skryptu automatycznej konfiguracji serwera proxy. Na przykład wprowadź `https://proxy.contoso.com/proxy.pac`.

    Aby uzyskać więcej informacji na temat plików PAC, zobacz [pliku automatycznej konfiguracji serwera proxy (PAC)](https://developer.mozilla.org/docs/Web/HTTP/Proxy_servers_and_tunneling/Proxy_Auto-Configuration_(PAC)_file) (otwiera witrynę firmy innej niż Microsoft).
...
- **Dodawanie i usuwanie kont**: wybierz pozycję **Blokuj**, aby uniemożliwić użytkownikom końcowym ręczne dodawanie lub usuwanie kont w profilu służbowym. Na przykład w przypadku wdrożenia aplikacji Gmail w profilu służbowym systemu Android można uniemożliwić użytkownikom końcowym dodawanie i usuwanie kont w tym profilu służbowym. Pozycja **Nieskonfigurowane** zezwala na dodawanie kont w profilu służbowym.  
  ...
  > [!NOTE]
  > Kont Google nie można dodać do profilu służbowego.
...
- **Zezwalaj na widżety z aplikacji profilu służbowego**: **Włącz** umożliwia użytkownikom końcowym umieszczanie widżetów uwidocznionych przez aplikacje na ekranie głównym. **Nie skonfigurowano** (ustawienie domyślne) — wyłącza tę funkcję.

  Na przykład program Outlook jest instalowany w profilach służbowych użytkowników. Gdy ustawienie **Włącz**, użytkownicy będą mogli umieścić widżet porządku na ekranie głównym urządzenia.
...
- **Zapobiegaj instalacji aplikacji z nieznanych źródeł w profilu osobistym**: według projektu, urządzenia profilu służbowego systemu Android Enterprise nie mogą instalować aplikacji ze źródeł innych niż Sklep Play. Ze względu na to, że urządzenia profilu służbowego są przeznaczone do podwójnego profilu:
  ...
  To ustawienie umożliwia administratorom większą kontrolę nad instalacjami aplikacji z nieznanych źródeł. **Nie skonfigurowano** (domyślnie) umożliwia instalowanie aplikacji z nieznanych źródeł w profilu osobistym. **Blokuj** uniemożliwia instalacje aplikacji ze źródeł innych niż Sklep Play w profilu osobistym.
