...
Te ustawienia korzystają z [SHAREDPC CSP](https://docs.microsoft.com/windows/client-management/mdm/sharedpc-csp).
...
- **Limit czasu uśpienia (w sekundach)**: wpisz, po ilu sekundach (od 0 do 18 000) nieaktywności urządzenie przejdzie w stan uśpienia. `0` oznacza, że urządzenie nigdy nie jest w stanie uśpienia. Jeśli nie ustawisz czasu, urządzenie przejdzie w stan uśpienia po upływie 3600 sekund (60 minut).
...
- **Szybkie logowanie**: wybierz opcję **włączone** , aby użytkownicy mogli szybko korzystać z funkcji logowania jednokrotnego. Gdy ta **Funkcja jest włączona**, urządzenie automatycznie łączy nowe konta usługi Azure AD inne niż administracyjne z wstępnie skonfigurowanymi kontami lokalnymi kandydatów. Wybierz pozycję **wyłączone** , aby uniemożliwić szybkie logowanie. Domyślna opcja **Nieskonfigurowane** oznacza, że to ustawienie jest niezarządzane przez usługę Intune i nie wypycha żadnych zasad w celu kontroli tego ustawienia na urządzeniu.

  [Uwierzytelnianie/EnableFastFirstSignIn dostawcy CSP](https://docs.microsoft.com/windows/client-management/mdm/policy-csp-authentication#authentication-enablefastfirstsignin)
