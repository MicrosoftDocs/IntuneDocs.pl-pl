...
description: Przejrzyj wartości domyślne i dostępne ustawienia w punkcie odniesienia zabezpieczeń rozwiązania MDM dla urządzeń z systemem Windows 10, którymi zarządzasz za pomocą usługi Intune.
author: brenduns
ms.author: brenduns
manager: dougeby
ms.date: 02/03/2020
...
Wyświetl ustawienia punktów odniesienia zabezpieczeń rozwiązania MDM, które obsługuje usługa Microsoft Intune dla urządzeń z systemem Windows 10 lub nowszym. Wartości domyślne ustawień w tym punkcie odniesienia reprezentują zalecaną konfigurację dla odpowiednich urządzeń i mogą nie być zgodne z wartościami domyślnymi innych punktów odniesienia zabezpieczeń.  

Najnowsza wersja punktu odniesienia to **Punkt odniesienia zabezpieczeń rozwiązania MDM z maja 2019 r.**  

Aby dowiedzieć się więcej o zmianach w najnowszej wersji tego punktu odniesienia, zobacz [Co zmieniło się w nowym szablonie](#whats-changed-in-the-new-template).  

> [!NOTE]  
> Wersja zapoznawcza punktu odniesienia zabezpieczeń rozwiązania MDM została zastąpiona w czerwcu 2019 r. przez wydanie szablonu *Punkt odniesienia zabezpieczeń rozwiązania MDM z maja 2019 r.* , który jest ogólnie dostępny (nie w wersji zapoznawczej). Profile utworzone przed udostępnieniem szablonu *Punkt odniesienia zabezpieczeń rozwiązania MDM z maja 2019 r.* nie zostaną zaktualizowane w celu odzwierciedlenia ustawień i wartości dostępnych w wersji punktu odniesienia zabezpieczeń rozwiązania MDM z maja 2019 r.  Nie można tworzyć nowych profilów opartych na szablonie w wersji zapoznawczej, ale można edytować wcześniej utworzone profile oparte na szablonie w wersji zapoznawczej oraz nadal korzystać z tych profilów. 
  
Aby dowiedzieć się więcej o używaniu punktów odniesienia zabezpieczeń z usługą Intune, zobacz [Używanie punktów odniesienia zabezpieczeń](security-baselines.md).  
...
- **Blokuj instalowanie aplikacji z podniesionymi uprawnieniami**  
...
- **Skonfiguruj bezpieczny dostęp do ścieżek UNC**  
  ...
  **Domyślne**: Skonfiguruj system Windows w celu zezwalania na dostęp do określonych ścieżek UNC tylko po spełnieniu dodatkowych wymagań dotyczących zabezpieczeń
  ...
  Gdy jest wybrane ustawienie *Skonfiguruj system Windows w celu zezwalania na dostęp do określonych ścieżek UNC tylko po spełnieniu dodatkowych wymagań dotyczących zabezpieczeń*, można skonfigurować listę ścieżek UNC ze wzmocnionymi zabezpieczeniami.
...
- **Blokuj usługę Windows Hello dla firm**  
  ...
  - W przypadku wybrania opcji *Wyłącz* urządzenia aprowizują usługę Windows Hello dla firm.
  - W przypadku wybrania opcji *Nieskonfigurowane* punkt odniesienia nie wpływa na ustawienie zasad urządzenia. Oznacza to, że bieżący stan usługi Windows Hello dla firm na urządzeniu pozostaje bez zmian.

  Można wyłączyć funkcję Windows Hello dla firm podczas konfigurowania [rejestracji w systemie Windows](windows-hello.md) lub w ramach profilu konfiguracji urządzenia dla [ochrony tożsamości](identity-protection-configure.md).  

  - **Ustawienie domyślne**: Włączono

  Następujące ustawienia są dostępne, gdy to ustawienie ma wartość *Wyłączone*.
  ...
  - **Małe litery w numerze PIN**  
    W przypadku opcji *Wymagane* numer PIN użytkownika będzie musiał zawierać co najmniej jedną małą literę.

    **Ustawienie domyślne**: Dozwolone

  - **Znaki specjalne w numerze PIN**  
    W przypadku opcji *Wymagane* numer PIN użytkownika będzie musiał zawierać co najmniej jeden znak specjalny.
  ...
  - **Wielkie litery w numerze PIN**  
    W przypadku opcji *Wymagane* numer PIN będzie musiał zawierać co najmniej jedną wielką literę.
...
- **Internet Explorer: automatyczny monit dla pobierania plików w strefie internetowej** > teraz jest używana wartość domyślna **Wyłączone**. W wersji zapoznawczej ustawienie to miało wartość Wyłączone.
...
*[Nowe]* [**Zapora Microsoft Defender**](#microsoft-defender-firewall)
- **Profil zapory: domena**  
  - **Połączenia przychodzące zablokowane**  
  - **Połączenia wychodzące wymagane**  
  - **Powiadomienia przychodzące zablokowane**  
