...
description: Przejrzyj ustawienia domyślne i dostępne dla różnych wersji linii bazowej zabezpieczeń systemu Windows MDM, którymi można zarządzać za pomocą Microsoft Intune.
author: brenduns
ms.author: brenduns
manager: dougeby
ms.date: 12/05/2019
...
Wyświetl ustawienia linii bazowej zabezpieczeń MDM, które Microsoft Intune obsługiwane w przypadku urządzeń z systemem Windows 10 lub nowszym. Wartości domyślne dla ustawień w tej linii bazowej reprezentują zalecaną konfigurację dla odpowiednich urządzeń. Wartości domyślne dla jednej linii bazowej mogą nie być zgodne z ustawieniami domyślnymi z innych linii bazowych zabezpieczeń lub z innych wersji tego punktu odniesienia.

- Aby dowiedzieć się więcej o korzystaniu z linii bazowych zabezpieczeń w usłudze Intune i jak uaktualnić wersję bazową w profilach odniesienia zabezpieczeń, zobacz [Korzystanie z linii bazowych zabezpieczeń](../security-baselines.md).
- Najnowsza wersja linii bazowej to **linia bazowa zabezpieczeń MDM dla 2019 maja**

Upewnij się, że wybrano wersję linii bazowej, która ma zostać wyświetlona.
...
> [!NOTE]
> W czerwcu 2019 *linia bazowa zabezpieczeń MDM dla z maja 2019* szablon została wydana jako ogólnie dostępna (nie w wersji zapoznawczej). Ta wersja linii bazowej zabezpieczeń zastępuje poprzednią linię bazową, *linię bazową zabezpieczeń MDM dla października 2018*.  Profile, które zostały utworzone przed udostępnieniem planu bazowego 2019 maja, nie zostaną zaktualizowane w celu odzwierciedlenia ustawień i wartości, które znajdują się w wersji 2019 maja.  Chociaż nie można tworzyć nowych profilów opartych na szablonie wersji zapoznawczej, można edytować i nadal używać wcześniej utworzonych profilów, które są oparte na szablonie wersji zapoznawczej.

Aby dowiedzieć się więcej o zmianach w tej wersji linii bazowej z poprzedniej wersji, zobacz [co zmieniono w nowym szablonie](#whats-changed-in-the-new-template).

::: zone-end
::: zone pivot="mdm-preview"
**Wersja zapoznawcza — punkt odniesienia zabezpieczeń oprogramowania MDM z października 2018 r.**:  
> [!NOTE]
> Jest to wersja zapoznawcza linii bazowej zabezpieczeń MDM wydaną w październiku 2018. Ta linia bazowa w wersji zapoznawczej została zastąpiona w czerwcu 2019 przez wydanie *linii bazowej zabezpieczeń MDM dla 2019 programu* , która jest ogólnie dostępna (nie w wersji zapoznawczej). Profile, które zostały utworzone przed udostępnieniem *linii bazowej zabezpieczeń MDM dla 2019 maja* , nie zostaną zaktualizowane w celu odzwierciedlenia ustawień i wartości, które znajdują się w linii bazowej zabezpieczeń MDM dla maja 2019 wersji. Chociaż nie można tworzyć nowych profilów opartych na szablonie wersji zapoznawczej, można edytować i nadal używać wcześniej utworzonych profilów, które są oparte na szablonie wersji zapoznawczej.
...
- **Blokuj instalacje aplikacji MSI z podniesionymi uprawnieniami**:  
  ...
  - *Włączenie tego ustawienia zasad*spowoduje, że uprawnienia zostaną rozszerzone na wszystkie programy. Zazwyczaj te uprawnienia są zastrzeżone dla programów przypisanych do użytkownika (oferowanych na pulpicie), przypisanych do komputera (instalowanych automatycznie) lub dostępnych w aplecie Dodaj lub usuń programy w panelu sterowania. To ustawienie profilu umożliwia użytkownikom instalowanie programów, które wymagają dostępu do katalogów, do których użytkownik może nie mieć uprawnień do wyświetlania lub zmieniania, w tym katalogów na komputerach z wysoce ograniczonymi uprawnieniami.

  - *Jeśli to ustawienie zasad zostanie wyłączone lub nie zostanie skonfigurowane*, system zastosuje uprawnienia bieżącego użytkownika podczas instalacji programów niedystrybuowanych lub oferowanych przez administratora systemu. Uwaga: to ustawienie zasad jest dostępne w folderach Konfiguracja komputera i Konfiguracja użytkownika. Aby to ustawienie zasad zaczęło obowiązywać, należy włączyć je w obu folderach. Uwaga: Użytkownicy z wykwalifikowanymi użytkownikami mogą korzystać z uprawnień tych ustawień zasad, aby zmienić ich uprawnienia i uzyskać stały dostęp do ograniczonych plików i folderów. Nie ma gwarancji, że wersja konfiguracji użytkownika tego ustawienia zasad jest bezpieczna.  
...
- **Skonfiguruj bezpieczny dostęp do ścieżek UNC**:  
  ...
  **Domyślnie**: Skonfiguruj system Windows tak, aby zezwalał na dostęp do określonych ścieżek UNC po spełnieniu dodatkowych wymagań w zakresie zabezpieczeń.
  ...
  Jeśli wybrano opcję *zezwalania na dostęp tylko do określonych ścieżek UNC po skonfigurowaniu systemu Windows* , można skonfigurować *ścieżkę UNC ze wzmocnionymi*zabezpieczeniami.
...
- **Wymagaj, aby klient zawsze podpisuj cyfrowo komunikację**:  
...
- **Poziom uwierzytelniania**:  
  ...
  - *Wyślij LM i NTLM-NTLMv2 w przypadku negocjowania* — klienci używają uwierzytelniania LM i NTLM i używają zabezpieczeń sesji NTLMv2, jeśli serwer go obsługuje. Kontrolery domeny akceptują uwierzytelnianie LM, NTLM i NTLMv2.
  ...
  - *Wysyłaj tylko odpowiedź NTLMv2. Odmowa LM*-klienci korzystają tylko z uwierzytelniania NTLMv2 i używają zabezpieczeń sesji NTLMv2, jeśli serwer ją obsługuje. Kontrolery domeny odrzucają LM (akceptują tylko uwierzytelnianie NTLM i NTLMv2).

  - *Wysyłaj tylko odpowiedź NTLMv2. Odmowa* LM i NTLM — klienci używają tylko uwierzytelniania NTLMv2 i używają zabezpieczeń sesji NTLMv2, jeśli serwer ją obsługuje. Kontrolery domeny odrzucają LM i NTLM (akceptują tylko uwierzytelnianie NTLMv2).
...
- **Żądanie pomocy zdalnej**:  
  ...
  **Domyślne**: Wyłącz pomoc zdalną
  ...
  - **Metoda zaproszenia do wiadomości E-mail**:  
...
- **Kod makr w pakiecie Office zezwala na typ importu elementów Win32**:  
  Złośliwe oprogramowanie może używać kodu makra w plikach pakietu Office, aby importować i ładować biblioteki DLL Win32. Kod ten jest następnie używany do wykonywania wywołań interfejsu API w celu dalszego zainfekowania systemu. Ta reguła podejmuje próbę zablokowania plików pakietu Office, które zawierają kod makro, który może importować biblioteki Win32 DLL. Dotyczy to też programów Word, Excel, PowerPoint i OneNote.  
...
- **Skonfiguruj usługę Windows Hello dla firm**
  ...
  - Po ustawieniu na *wartość Yes (tak*) punkt odniesienia nie ma wpływu na ustawienie zasad urządzenia. Oznacza to, że jeśli funkcja Windows Hello dla firm została wyłączona na urządzeniu, zostanie wyłączona. Jeśli jest włączona, pozostanie włączona.
  <!-- expected behavior 
...
*[New]* [**powyżej blokady**](#above-lock):
...
- Zasada dysku wymiennego funkcji BitLocker > **Metoda szyfrowania**
- **Zasady dysków stałych funkcji BitLocker** *(wszystkie ustawienia)*
- **Zasady dysku systemowego funkcji BitLocker** *(wszystkie ustawienia)*
...
- **Automatyczna Monituj o pobieranie plików w strefie internetowej programu Internet Explorer** > wartość domyślna jest teraz **wyłączona**. W wersji zapoznawczej to ustawienie miało wartość włączone.
