...
- [Urząd zarządzania urządzeniami przenośnymi](../fundamentals/mdm-authority-set.md)
- [Certyfikat wypychania MDM firmy Apple](../enrollment/apple-mdm-push-certificate-get.md)
...
1. W [centrum administracyjnym usługi Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431) wybierz pozycję **Urządzenia** > **macOS** > **Rejestracja urządzeń z systemem macOS** > **Tokeny programu rejestracji** > **Dodaj**.

    ![Pobierz token programu rejestracji.](./media/device-enrollment-program-enroll-macos/image01.png)
...
2. Udziel firmie Microsoft uprawnień do wysyłania informacji o użytkowniku i urządzeniu do firmy Apple, wybierając pozycję **Zgadzam się**.
   ...
   ![Zrzut ekranu przedstawiający okienko tokenu programu Enrollment Program w obszarze roboczym certyfikatów firmy Apple umożliwiające pobranie klucza publicznego.](./media/device-enrollment-program-enroll-macos/add-enrollment-program-token-pane.png)
...
### <a name="step-2-use-your-key-to-download-a-token-from-apple"></a>Krok 2. Użyj swojego klucza do pobrania tokenu Apple
...
7. W obszarze **Choose Devices By** (Sposób wybierania urządzeń) określ sposób identyfikowania urządzeń:
   ...
   ![Zrzut ekranu przedstawiający wybraną opcję wybierania urządzeń według numerów seryjnych, akcję po wybraniu ustawioną jako przypisywanie do serwera oraz wybraną nazwę serwera.](./media/device-enrollment-program-enroll-macos/enrollment-program-token-specify-serial.png)
...
![Zrzut ekranu przedstawiający wprowadzanie identyfikatora Apple ID używanego do utworzenia tokenu programu rejestracji i przechodzenie do tokenu programu rejestracji.](./media/device-enrollment-program-enroll-macos/image03.png)
...
1. W [centrum administracyjnym usługi Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431) wybierz pozycję **Urządzenia** > **macOS** > **Rejestracja urządzeń z systemem macOS** > **Tokeny programu rejestracji**.
2. Wybierz token, wybierz pozycję **Profile**, a następnie wybierz pozycję **Utwórz profil**.

    ![Utwórz zrzut ekranu profilu.](./media/device-enrollment-program-enroll-macos/image04.png)
...
8. Wybierz pozycję **Ustawienia Asystenta ustawień**, aby skonfigurować następujące ustawienia profilu:  ![Dostosowanie Asystenta ustawień.](./media/device-enrollment-program-enroll-macos/setupassistantcustom-macos.png)
    ...
    | <strong>Magazyn iCloud</strong> | Umożliwiać użytkownikowi korzystanie z magazynu iCloud. |    
    ...
    | <strong>Czas przed ekranem</strong>| Wyświetlać ekran Czas korzystania z urządzenia. |

9. Wybierz przycisk **OK**.

10. Abu zapisać profil, wybierz pozycję **Utwórz**.
...
1. W [centrum administracyjnym programu Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431) wybierz pozycję **Urządzenia** > **macOS** > **Rejestracja urządzeń z systemem macOS** > **Tokeny programu rejestracji**, wybierz token na liście, a następnie wybierz pozycję **Urządzenia** > **Synchronizuj**. ![Zrzut ekranu przedstawiający wybrany węzeł Urządzenia programu Enrollment Program i wybierany link Synchronizuj.](./media/device-enrollment-program-enroll-macos/image06.png)
...
1. W [centrum administracyjnym usługi Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431) wybierz pozycję **Urządzenia** > **macOS** > **Rejestracja urządzeń z systemem macOS** > **Tokeny programu rejestracji** > wybierz token na liście.
...
Można wybrać profil domyślny w systemie macOS lub iOS/iPadOS, który zostanie zastosowany do wszystkich urządzeń rejestrowanych przy użyciu określonego tokenu. 
...
3. Wybierz pozycję **Wygeneruj nowy token**.
    ...
    ![Zrzut ekranu przedstawiający generowanie nowego tokenu.](./media/device-enrollment-program-enroll-macos/generatenewtoken.png)

4. Wybierz pozycję **Token Twojego serwera**.  
5. W [centrum administracyjnym programu Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431) wybierz pozycję **Rejestrowanie urządzenia** > **Rejestracja Apple** > **Tokeny programu rejestracji** > wybierz token.
    ![Zrzut ekranu przedstawiający tokeny programu rejestracji.](./media/device-enrollment-program-enroll-macos/enrollmentprogramtokens.png)

6. Wybierz pozycję **Odnów token** i wprowadź identyfikator Apple ID użyty do utworzenia pierwotnego tokenu.  
    ![Zrzut ekranu przedstawiający generowanie nowego tokenu.](./media/device-enrollment-program-enroll-macos/renewtoken.png)

7. Przekaż nowo pobrany token.  
8. Wybierz pozycję **Odnów token**. Wyświetli się potwierdzenie, że token został odnowiony.
    ![Zrzut ekranu przedstawiający potwierdzenie.](./media/device-enrollment-program-enroll-macos/confirmation.png)

## <a name="next-steps"></a>Następne kroki

Po zarejestrowaniu urządzeń z systemem macOS można rozpocząć [zarządzanie nimi](../remote-actions/device-management.md).
