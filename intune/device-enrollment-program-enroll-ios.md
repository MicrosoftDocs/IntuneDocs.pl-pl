...
Włączenie rejestracji w programie DEP wymaga użycia zarówno portalu usługi Intune, jak i portalu DEP firmy Apple. Wymagana jest również lista numerów zamówień zakupu urządzeń objętych programem DEP, która pozwala przypisać te urządzenia do funkcji zarządzania usługi Intune w portalu firmy Apple.
...
1. [Pobieranie tokenu DEP firmy Apple i przypisywanie urządzeń](#get-the-apple-dep-certificate)
2. [Tworzenie profilu rejestracji](#create-anapple-enrollment-profile)
3. [Synchronizowanie urządzeń zarządzanych w programie DEP](#sync-dep-managed-devices)
4. [Przypisywanie profilu usługi DEP do urządzeń](#assign-a-dep-profile-to-devices)
5. [Przekazywanie urządzeń użytkownikom](#distribute-devices-to-users)
...
Wybierz pozycję **Utwórz token za pomocą programu Device Enrollment Program firmy Apple**, aby otworzyć portal programu wdrażania firmy Apple i zalogować się przy użyciu firmowego identyfikatora Apple ID. Tego identyfikatora firmy Apple możesz użyć do odnowienia tokenu DEP.
  ...
  ![Zrzut ekranu przedstawiający okienko tokenu programu Enrollment Program w obszarze roboczym certyfikatów firmy Apple umożliwiające pobranie klucza publicznego.](./media/enrollment-program-token-sign.png)
   1.  W [portalu programów wdrażania](https://deploy.apple.com) firmy Apple wybierz pozycję **Get Started** (Rozpocznij) dla opcji **Device Enrollment Program**.
   ...
   2. Na stronie **Manage Servers** (Zarządzanie serwerami) wybierz pozycję **Add MDM Server** (Dodaj serwer MDM).
   ...
   4.  Zostanie otwarte okno dialogowe **Add &lt;nazwa_serwera&gt;** (Dodawanie serwera <nazwa_serwera>) z widocznym komunikatem **Upload Your Public Key** (Przekaż klucz publiczny). Kliknij pozycję **Choose File…** (Wybierz plik...) w celu przekazania pliku PEM, a następnie kliknij przycisk **Next** (Dalej).
   ...
   5. Wybierz kolejno pozycje **Deployment Programs** (Programy wdrażania)&gt; **Device Enrollment Program** (Program rejestracji urządzeń) &gt; **Manage Devices** (Zarządzaj urządzeniami).
   6. W obszarze **Choose Devices By** (Sposób wybierania urządzeń) określ sposób identyfikowania urządzeń:
...
4. Wybierz pozycję **Ustawienia zarządzania urządzeniami**, aby skonfigurować następujące ustawienia profilu: ![Zrzut ekranu przedstawiający wybieranie trybu zarządzania z ustawieniami Nadzorowane i Rejestracja zablokowana oraz wybraną opcją Odmawiaj wszystkim ustawienia Zezwalaj na parowanie, a także wyszarzonym ustawieniem Certyfikaty programu Apple Configurator dla nowego profilu programu rejestracji.](./media/enrollment-program-profile-mode.png)
...
5. Wybierz pozycję **Ustawienia Asystenta ustawień**, aby skonfigurować następujące ustawienia profilu: ![Zrzut ekranu przedstawiający wybieranie pozycji konfigurowania ustawień oraz dostępne ustawienia dla nowego profilu programu rejestracji.](./media/enrollment-program-profile-settings.png)
...
Przed zarejestrowaniem urządzeń zarządzanych przez usługę Intune musisz przypisać do nich profil programu rejestracji.
...
2. Użytkownicy widzą komunikat: **Firma Microsoft automatycznie skonfiguruje urządzenie.** Dostępne są również następujące szczegółowe informacje dotyczące konfiguracji:

  **Konfiguracja umożliwia firmie Microsoft zarządzanie tym urządzeniem bez udziału użytkownika.**
  ...
  **Dostawcą konfiguracji jest: <br> Zespół usługi Microsoft Intune dla systemu iOS<br> One Microsoft Way, Redmond, WA 98052 (USA)**
