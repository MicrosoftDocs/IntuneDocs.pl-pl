...
- **Łącznik certyfikatów PFX dla usługi Microsoft Intune**: jeśli zamierzasz używać szyfrowania wiadomości e-mail za pomocą protokołu S/MIME, należy użyć witryny Azure Portal do pobrania instalatora **łącznika certyfikatów PFX dla usługi Microsoft Intune** ( **PfxCertificateConnectorBootstrapper.exe**). Łącznik przetwarza żądania na potrzeby plików PFX zaimportowanych w usłudze Intune dla protokołu szyfrowania wiadomości e-mail za pomocą protokołu S/MIME dla określonego użytkownika.

- **Windows Server**: hostuje:
...
14. W przypadku serwera do zarządzania certyfikatami w imieniu urządzeń i użytkowników zarejestrowanych w usłudze Intune wykonaj następujące kroki:
    ...
    2. Na karcie Zabezpieczenia dodaj konto komputera serwera, na którym uruchamiasz łączniki (**Łącznik certyfikatów usługi Microsoft Intune** lub **Łącznik certyfikatów PFX dla usługi Microsoft Intune**). Dla konta komputera przydziel uprawnienia **Wystawianie certyfikatów i zarządzanie nimi** i **Żądaj certyfikatów**.
...
4. Wybierz pozycję **Dodaj**, a następnie pobierz łącznik certyfikatów usługi Microsoft Intune. Zapisz pobraną zawartość w lokalizacji dostępnej z serwera, na którym zostanie ona zainstalowana.
...
7. Na karcie **Zaawansowane** zalecamy pozostawienie zaznaczonej pozycji **Użyj konta SYSTEM na tym komputerze (domyślnie)**.
...
2. Wybierz opcję **Wszystkie usługi**, odfiltruj usługę **Intune**, a następnie wybierz pozycję **Microsoft Intune**.
3. Wybierz pozycję **Konfiguracja urządzenia**, a następnie pozycję **Urząd certyfikacji**.
4. Wybierz pozycję **Dodaj**, a następnie pobierz łącznik certyfikatów PFX dla usługi Microsoft Intune. Zapisz pobraną zawartość w lokalizacji dostępnej z serwera, na którym zostanie ona zainstalowana.
5. Po zakończeniu pobierania zaloguj się na serwerze. Następnie:
...
6. Łącznik certyfikatów PFX dla usługi Microsoft Intune otwiera kartę **Rejestracja** po zakończeniu instalacji. Aby włączyć połączenie z usługą Intune, **zaloguj się** i wprowadź nazwę konta z globalnymi uprawnieniami administratora platformy Azure lub uprawnieniami administratora usługi Intune.
7. Zamknij okno.
