...
| Zasady ochrony aplikacji nie działają z programem DEP | Zasady ochrony aplikacji nie są stosowane do urządzeń korzystających z programu DEP firmy Apple. | Upewnij się, że używasz koligacji użytkownika na potrzeby programu Device Enrollment Program (DEP) firmy Apple. Koligacja użytkownika jest wymagana dla każdej aplikacji, która wymaga uwierzytelniania użytkownika w ramach programu DEP. <br><br>Artykuł [Automatyczne rejestrowanie urządzeń z systemem iOS/iPadOS w ramach programu Device Enrollment Program firmy Apple](../enrollment/device-enrollment-program-enroll-ios.md) zawiera dalsze informacje na temat rejestracji w ramach programu DEP dla systemu iOS/iPadOS.|
| Zasady transferu danych nie działają w systemie iOS/iPadOS | Zasady **Zezwalaj aplikacji na transfer danych do innych aplikacji** i **Zezwalaj aplikacji na odbieranie danych z innych aplikacji** nie zarządzają pomyślnie transferem danych w systemie iOS/iPadOS. | Zobacz temat [Jak zarządzać przesyłaniem danych między aplikacjami systemu iOS/iPadOS w usłudze Microsoft Intune](data-transfer-between-apps-manage-ios.md). |
...
---| --- | --- |
iOS | Użytkownik końcowy może otwierać dane służbowe w aplikacjach niezarządzanych przy użyciu rozszerzenia udostępniania systemu iOS/iPadOS nawet wtedy, gdy zasady transferu danych mają wartość **Tylko aplikacje zarządzane** lub **Brak aplikacji**. Czy te dane nie wyciekają? | Zasady ochrony aplikacji usługi Intune nie mogą kontrolować rozszerzenia udostępniania systemu iOS/iPadOS bez zarządzania danym urządzeniem. W związku z tym **usługa Intune szyfruje dane „firmowe” przed ich udostępnieniem poza aplikację**. Aby to sprawdzić, spróbuj otworzyć plik „firmowy” poza aplikacją zarządzaną. Plik powinien być zaszyfrowany i jego otwarcie poza zarządzaną aplikacją nie powinno być możliwe.
iOS | Dlaczego użytkownik końcowy **widzi monit o zainstalowanie aplikacji Microsoft Authenticator** | Jest to konieczne w przypadku stosowania dostępu warunkowego opartego na aplikacji. Zobacz temat [Wymaganie zatwierdzonej aplikacji klienckiej](https://docs.microsoft.com/azure/active-directory/conditional-access/app-based-conditional-access).
...
iOS/Android | Zasady ochrony aplikacji nie zostały zastosowane w wersji roboczej wiadomości e-mail w aplikacji Outlook | Ponieważ program Outlook obsługuje zarówno kontekst firmowy, jak i prywatny, nie wymusza stosowania funkcji MAM w wiadomościach e-mail w wersji roboczej.
iOS/Android | Zasady ochrony aplikacji nie są stosowane do nowych dokumentów w programach WXP (Word, Excel, PowerPoint) | Ponieważ programy WXP obsługują zarówno kontekst firmowy, jak i prywatny, nie wymuszają one funkcji MAM w nowych dokumentach, dopóki dokumenty te nie zostaną zapisane w zidentyfikowanej lokalizacji firmowej, takiej jak usługa OneDrive.
iOS/Android | Aplikacje nie zezwalają na zapisywanie jako do magazynu lokalnego po włączeniu zasad | Zachowanie aplikacji dla tego ustawienia jest kontrolowane przez dewelopera aplikacji.
Android | System Android ma więcej ograniczeń niż systemy iOS/iPadOS, w przypadku których aplikacje „natywne” mogą uzyskiwać dostęp do zawartości chronionej przy użyciu funkcji MAM | System Android to platforma otwarta, a użytkownik końcowy może zmienić skojarzenie aplikacji „natywne” na aplikacje potencjalnie niebezpieczne. Zastosuj [wyjątki od zasad transferu danych](app-protection-policies-exception.md), aby wykluczyć określone aplikacje.
Android | Usługa Azure Information Protection (AIP) może zapisywać jakop lik PDF, gdy nie można używać pozycji Zapisz jako | Usługa AIP stosuje zasady MAM dla pozycji „Wyłącz drukowanie” w przypadku korzystania z opcji drukowania jako PDF.
iOS | Otwieranie załączników PDF w aplikacji Outlook kończy się niepowodzeniem z komunikatem „Akcja niedozwolona“ | Taka sytuacja może wystąpić, jeśli użytkownik nie został uwierzytelniony w programie Acrobat Reader dla usługi Intune lub użył odcisku palca do uwierzytelnienia w organizacji. Otwórz program Acrobat Reader wcześniej i uwierzytelnij się przy użyciu poświadczeń nazwy UPN.
...
- Informacje na temat rozwiązywania problemów z zasadami rozwiązania Intune App Protection przy użyciu plików dzienników można znaleźć w artykule [https://techcommunity.microsoft.com/t5/Intune-Customer-Success/Support-Tip-Troubleshooting-Intune-app-protection-policy-using/ba-p/330372](https://techcommunity.microsoft.com/t5/Intune-Customer-Success/Support-Tip-Troubleshooting-Intune-app-protection-policy-using/ba-p/330372)
